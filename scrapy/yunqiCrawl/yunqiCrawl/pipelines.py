...
class YunqicrawlPipeline(object):
    ...
    def __init__(self, mongo_uri, mongo_db,replicaset):
        self.mongo_uri = mongo_uri #定义mogodb host
        self.mongo_db = mongo_db #定义数据库
        self.replicaset = replicaset #定义mogodb集群副本集

    @classmethod
    def from_crawler(cls, crawler):
        return cls(
            mongo_uri=crawler.settings.get('MONGO_URI'), #从设置中获取
            mongo_db=crawler.settings.get('MONGO_DATABASE', 'yunqi'),
            replicaset = crawler.settings.get('REPLICASET')
        )

    def open_spider(self, spider):#当spider运行时打开
        self.client = pymongo.MongoClient(self.mongo_uri,replicaset=self.replicaset) #建立mongoda集群连接
        

    def close_spider(self, spider):#当spider关闭时运行
        self.client.close()
    ...
    def _process_booklist_item(self,item):
        ...
        :return:
        '''
        self.db.bookInfo.insert(dict(item))#插入数据到数据库
